...
const SelectAllPermissionsButton = ({ editablePermissions }: { editablePermissions: string[] }) => {
    const { values, setFieldValue } = useFormikContext<Values>();
    const allSelected = editablePermissions.length > 0 && editablePermissions.every((p) => values.permissions.includes(p));

    return (
        <div css={tw`flex justify-end mb-4`}>
            <button
                type={'button'}
                css={tw`text-sm px-3 py-1.5 rounded transition-colors duration-100`}
                style={{
                    backgroundColor: allSelected ? '#2a2a2a' : 'rgba(255, 76, 76, 0.1)',
                    color: allSelected ? '#a3a3a3' : '#FF4C4C',
                    border: `1px solid ${allSelected ? '#3a3a3a' : 'rgba(255, 76, 76, 0.3)'}`,
                    borderRadius: '2px',
                    cursor: 'pointer',
                }}
                onClick={() => {
        ...
        >
            <Form>
                ...
                {!isRootAdmin && loggedInPermissions[0] !== '*' && (
                    <div css={tw`mt-4 pl-4 py-2 border-l-4`} style={{ borderLeftColor: '#FF4C4C' }}>
                ...
                <div css={tw`my-6`}>
                    {canEditUser && <SelectAllPermissionsButton editablePermissions={editablePermissions} />}
